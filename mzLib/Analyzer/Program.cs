...
public class PlotFactory
{
    ...
    public PlotFactory(string[] paths, string[] labels)
    {
        ...
        for (int i = 0; i < paths.Length; i++)
        {
            ...
            fileDictionary.Add(label, psms.Results);
        }
    }
    public static void Main(string[] args)
    {
        CommandLineParser(args);
        var plotFactory = new PlotFactory(options.Where(x => x.Key == "--files").First().Value.ToArray(),
            options.Where(x => x.Key == "--labels").First().Value.ToArray());

        Run(options.Where(x => x.Key == "--tasks").First().Value.First());
    }
